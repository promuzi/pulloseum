...
### 2026-09-11 — 사용자 피드백: 함선 탭 잠금 복귀 · 상단 HUD 축소 · 설정 버튼 정리 · 상대 다음 수 비공개
- **함선 탭:** P0-D의 숨김 → **보이되 잠금**(사용자: "나중에 더 키울 거고 지금은 잠금"). `navbtn locked` + 🔒 배지, 탭하면 `openLockedSection('mode')` = 🚀 함선 · 준비 중 안내(`LOCKED_SECTIONS.mode` 문구 교체). `openShip`·함선 방 코드는 그대로 두고 핸들러만 잠금 안내로 — 열 때 핸들러를 `openShip()`으로 되돌리고 locked·배지 제거. 하단 탭 5개 복귀.
- **상단 HUD:** 숫자 24px("너무 크다") → **Galmuri14 15px·굵기 400**. 헤드리스로 크기별 글자 회색 단계를 세어 보니 12px(G11)·15px(G14)·24px(G11)만 번짐 없고 13·14·16·18px은 흐림 → 15px 채택. "Lv" 라벨도 숫자와 같은 굵기. 레벨·크레딧·젬 알약 높이 32px로 통일.
- **설정 버튼:** 48×48 박스가 혼자 컸음 → 보이는 알약 32×32(다른 HUD 알약과 같은 홀로 칩 룩, 아이콘 17→15px) + 투명 히트 영역 44×44.
- **검증:** 셀프테스트 136건(신규 2: 함선 탭 잠금·안내 제목 / 상대 다음 수 비공개 가드) — 실패는 기존 team 1건뿐. `tools/shots/audit.mjs` 전 화면 12px 미만 글자 0·44px 미만 탭 0 유지. 헤드리스로 함선 탭 탭 → 잠금 안내 열림·함선 방 안 열림 확인.
- **상대 다음 수 비공개로 되돌림**(사용자: "그런 거 하지 마, 나중에 PvP 할 거니까"): P0-C의 매 턴 의도 칩(`#eIntent`·`renderEnemyIntent`·`estimateHitDamage`·`enemyIntentValid`·`B.eNextIntent`) 전부 제거, `updateEnemyForecast`를 2026-07-02 원형(3코스트+ 큰 기술 55% ⚠️·종류 숨김)으로 복원, `submitIntent`는 다시 `aiPickIntent()` 직행(예고한 큰 기술은 거기서 지킴). 재도입 방지 셀프테스트 추가. 피격 연출·2배속·HP 이징 등 나머지 전투 피드백은 유지. 헤드리스 3턴 진행: 의도 칩 없음·페이지 에러 0. 교훈: 결정 로그에 기록된 사용자 결정(2026-07-02 "읽는 재미")을 설계 권고로 덮어쓰지 말 것.
...
- **HUD 숫자 24px:** 레벨·크레딧·젬 숫자만 24px(갈무리11 정수 2배), 단위 12px, 경험치 바 66×14.
- **히트 영역:** 설정 37×36→48, 슬롯 칩 55×43→60×48(프로필 배너 높이 자동), 관리창 ✕ 30→44, 종자 정렬·심기·판매 28→44, 탐사 확대/축소 38→48, 격납고 버튼 54(지도 scale .9 보정), 양육 전체 수확·항복 44, 토너먼트 셀 48, 하단 탭 52.
- **눌림 피드백:** 칩·카드·슬롯·셀·아이콘 버튼 공통 `:active` scale(.95)+밝기, `html{touch-action:manipulation}`, reduced-motion 시 전환 제거.
- **실측(신규 `tools/shots/audit.mjs`, 375×812 화면 9종):** 12px 미만 텍스트 8~40개 → **전 화면 0**, 44px 미만 탭 대상 1~8개 → **전 화면 0**, 픽셀 폰트 로드 확인. 셀프테스트 기존 1건 외 실패 없음. 전후 비교 = 세션 아티팩트 "풀로세움 P0-B 가독성".
- **사용자 피드백 반영:** 메인 스탯 8칸 두 열 폭이 달랐음(`1fr`의 min-content 확장) → `minmax(0,1fr)` + 레일 104→110 + 칸 flex 중앙 정렬(라벨·숫자). 실측 8칸 모두 54px, "에너지" 라벨 넘침 없음. 부작용: 중앙 열 131→119px라 6글자 이상 종 이름은 두 줄(P1 스탯 바 정리 때 해소).
- 남은 것(P1): 12.5~15px 중간 크기를 12·24 정수배로 정리, 메인 스탯 8칸→바 2개.
...
- **검증:** 셀프테스트 133건(신규 1: 신규 게이트·버튼 모드·2승 해금·지연 생성 항목 무시) — 실패 1 = 이전부터 있던 "team: loadout 6 + team state migration"(HEAD 동일, 별건). 첫 전투까지 탭 6→3.

### 2026-09-10 — UX 진단·설계 문서 (코드 무변경)
- **배경:** 사용자 요청 "UI·가독성·애니메이션·전환·판정·재미·직관성 전반을 전문가 눈으로 점검하고 업계 방식으로 설계". 375×812 신규 세이브로 화면 7종을 직접 플레이하며 스크립트 실측(탭 타깃 44px 미만 화면당 14~19개, 12px 미만 텍스트 40~65%, 7px 슬롯 라벨, 픽셀 폰트 파일 부재, 온보딩·배속 없음) + 병렬 조사 3각도(모바일 UX 기준 / 게임 필·전환 / 수집형 재미 루프·검증법).
- **문서:** [superpowers/specs/2026-09-10-ux-audit-and-design.md](superpowers/specs/2026-09-10-ux-audit-and-design.md). 결론 = 콘텐츠는 충분하나 화면이 "무엇을 왜 하는지"를 말하지 않는다. P0(테스트 전) = 첫 세션 재배선(잠금·배너 숨김, 심기 1탭, 강화 모달 자동 오픈 폐지, 첫 전투 CTA) / 가독성(갈무리 폰트 동봉·12px 하한·48px 히트 영역) / 전투 피드백(타이밍 재배분 4.0→2.6s, 플래시·히트스톱·데미지 숫자·HP 이징·2× 배속·매 턴 상대 의도) / 정리(IAP 카드 제거·함선 숨김·전환 fade-through).

### 2026-09-10 — 레트로 도트 스프라이트 5종 게임 반입 (렌더러 v2 산출)
- **반입:** `assets/sprites/{tree_fire,aqua,gaia,thorn,spore_cap}.png`(48×56 투명, 전 생장 단계 공용) + `pot_terra.png`. `SPRITE_OVERRIDES`·`POT_SPRITE_OVERRIDES.pot_terra` 등록.
- **정수배 표시:** `snapPixelSize()` — 요청 크기를 48의 배수로 스냅(140→144, 118/108/84/80→96, 48 미만은 그대로+부드럽게 축소). `spriteFor`는 도트 PNG일 때 화분도 같은 스냅 크기의 PNG로, 절차 SVG 식물일 땐 화분도 SVG(`potVisual(id,size,usePng)`).
- **산출 도구:** `tools/pixel/`(형태 레시피 `sprites-data.js` + 렌더러 `renderer-v2.js` + `out/`). 셀프테스트 1종 추가(스냅·오버라이드·화분 동반). 사용자 판정: v2(팔레트 통일·selout) "훨씬 낫다".
- 남은 것: 나머지 30종 레시피, 생장 단계별 그림(지금은 성체 1장 공용), 대기 프레임, 계단 균일화·내부 AA.

### 2026-09-10 — 스프라이트 아트 방향 확정 + 품질 조사 보고서 (코드 무변경)
- **배경:** 1주일 플레이스토어 출시 스프린트의 아트 병목(식물 개성) 해소. 사용자 결정: **레트로 도트 48×56, 표시 ×1/×2/×3(48·96·144), 외곽선 색별 어두운 톤, 눈은 2×2 점**(마스코트 카툰·초롱눈·실눈+눈썹+송곳니는 폐기). 5종 확정본(엠버오크·아쿠아벌브=고사리 새순 해마·가이아록·쏜위드·스포어캡)은 정수 좌표 도형 레시피로 작화(시안 페이지는 세션 아티팩트, PNG는 아직 저장소 미반입).
- **방법:** 모티프를 모르는 AI 판독기에 "뭐로 보이냐"를 묻는 블라인드 판독 루프로 범주 오류(코브라→버섯구름, 돌판→가면)를 잡았고, 심미 판정은 사용자(1차 형태 유지). 아쿠아벌브 코브라는 3회 실패 후 해마로 교체.
- **문서:** [sprite-quality-research.md](sprite-quality-research.md) — 기법·팔레트(Resurrect 64 속성 7램프)·벤치마크(포켓몬 Gen3·Cassette Beasts)·도구/AI/에셋·토큰 비용. 다음 = 렌더러 개선 8단계(광원·HSV 램프·selout·클린업) → 게임 반입(`SPRITE_OVERRIDES`, 표시 크기 144·96·48) → 애니(대기 프레임 자동 생성·공격/피격은 기존 transform).

### 2026-07-02 — 전투 UI 꿈틀/이상 배치 수정 (무대-스킬바 동기화 자가치유)
- **증상(사용자):** 전투 UI가 꿈틀거리고 이상함. **실측 원인:** `syncStageToSkillBar`가 스킬바 **빈/미배치 순간의 19px 측정값을 박제** → 무대(`#battleArena`) 바닥이 스킬바 뒤로 300px 늘어져 **내 식물이 스킬바에 깔림**(pTop 622 vs 바 top 404). 이후 리프레시마다 정상/비정상 값이 오가며 스냅 이동(꿈틀).
- **수정:** ① 오프셋 <60px 측정은 무시 + 더블 rAF 재보정(자가치유) ② `#battleArena`에 `transition:bottom .28s`(보정이 스냅 대신 슬라이드) ③ 리사이즈 시 재동기화 ④ ⚠️예고 펄스를 크기 확대→**밝기만**(1.6s)으로 톤다운(꿈틀 인상 제거).
- **검증:** 전투 한 턴 관통 36샘플 — 무대바닥=바높이 불일치 0회, 식물 위치 정상 복구, 셀프테스트 0 fail.

### 2026-07-02 — 방향 확정: 1v1 주력 회귀 (6스킬+변이 바 복원 · 3v3=보조 리그)
- **사용자 판단:** 한 식물에 들어가는 정성(변이·스탯 강화·스킬)이 게임의 축 → 3v3 주력은 그 정성을 3배로 분산시키는 구조적 모순. **1v1 주력 확정, 재미는 다른 부분(스킬 구동·메타 루프)에서 찾기로.**
- **복원:** `MAX_LOADOUT` 8→**6**(기존 세이브는 normalizeState slice로 자동 정리) + **변이 스킬 별도 바**(`#cardPred`) 복원. 메인 버튼 "⚔️ 토너먼트 시작"(=1v1).
- **존치(이번 공사의 수확):** 봇 성향 AI+⚠️큰기술 예고(1v1의 "읽는 재미"), 에너지 커브(시작2·+1/턴), 의도 수집·해소 엔진(PvP 대비), 팀 인프라(미션 보스+호위전·**3v3 보조 리그**로 유지 — 유지비 0), 재배사 랭크, 숨 고르기 폴백.
- 검증: 셀프테스트 0 fail · preview 1v1 실전(일반 4+변이 2 분리 렌더·턴 해소·에너지 커브).

### 2026-07-02 — 플레이테스트 피드백 반영: 손패 순환 폐지 + 팀전 진입 버그 수정
- **사용자 플레이테스트 지적 3건:** ① 3v3 팀전 클릭 시 전투 시작과 동시에 "참가 가능 경기" 모달이 뜸 ② 덱 순환이 별로 — 스택 쌓는 플레이가 안 됨 ③ 숨 고르기 필요를 못 느낌.
- **① 버그:** `#tournamentBar` 위임 리스너가 **모든 `.tcell` 클릭**에 `openMatches()`를 열고 있었음(3v3 리그·미션 행도 `.tcell`) → `.tcell[data-matches]`로 한정.
- **②③ 설계 번복 — 손패4 덱 순환 폐지:** 턴제에서 스택/콤보 셋업(비늘 강화·브레스 충전·태그 콤보·독 묻히기→공격)은 "특정 스킬을 특정 타이밍에" 써야 성립하는데 랜덤 손패가 이를 정확히 차단 — CR 사이클은 실시간용 메커니즘이었음을 실플레이로 확인. **스킬 8장 상시 오픈으로 회귀**(`unitDeckIds` 전체 렌더·3열 그리드), `makeDeckState/deckPlay/deckNext`·다음 카드 미리보기 제거. **숨 고르기는 상시 버튼 제거** → 사용 가능 스킬이 0일 때만 폴백 노출(0코 기본기 있으면 사실상 안 보임). **유지:** 팀3·교체 심리전·공유 에너지 커브(시작2·+1/턴·최대8 — 긴장 담당)·봇 성향/⚠️예고·두 리그. 결정론 가드 테스트 함수 목록 갱신. 셀프테스트 0 fail·preview 검증(팀전 진입 클린·8장 표시·모달 누수 없음).
...
- **#1-A 아키타입 실효과(fx):** 14종 전부 `ARCHETYPES[*].fx` + `__applyArchFx`(멱등) — 치명/관통/흡혈/에너지흡수/버프·디버프 부여·증폭/위력/회복을 `ind.*` 고유스킬에 실반영, desc에 【아키타입】 자동 표기. "광폭과 도사가 똑같이 때림" 해소.
- **밸런스 §8 3건:** 불/바람 `ELEMENT_STATS` 상향(+7/+6, 격차 42→35) · 무기 등급 위력 60% 완충(S 내려찍기 409→331) · 단일/광역 명중 방향 반전(단일+7·광역+0, 1:1 역설계 수정).
- **폴리시:** `josa()` 한국어 조사 유틸 + 깨진 문구 8곳 수정("감사용를"→"감사용을", "방어이"→"방어가").
- **문서:** 로드맵 표 #10 stale 정정(실제론 2차 개편까지 완료)·#15 완료 반영·§4.6 갱신·balance-sheet §8 취소선·§1-2 표 갱신.
- **검증:** `__catalogSelfTest()` **0 fail**(신규 테스트 5종 포함: josa·아키타입 fx·미션 데이터 정합·결정론+기믹·released 우회), preview E2E 전 구간 JS 에러 0.
