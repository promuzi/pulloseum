...
### 2026-09-11 — 사용자 피드백: 함선 탭 잠금 복귀 · 상단 HUD 축소 · 설정 버튼 정리
...
- **검증:** 셀프테스트 135건(신규 1: 함선 탭 잠금·안내 제목) — 실패는 기존 team 1건뿐. `tools/shots/audit.mjs` 전 화면 12px 미만 글자 0·44px 미만 탭 0 유지. 헤드리스로 함선 탭 탭 → 잠금 안내 열림·함선 방 안 열림 확인.
- 보류(사용자 질문 "상대 다음 공격은 왜 보여주냐"): P0-C의 매 턴 의도 표시는 이전 결정(⚠️ 큰 기술만·종류 숨김, 2026-07-02 "1v1 읽는 재미")을 덮어쓴 것이라 되돌릴지 사용자 판단 대기.
