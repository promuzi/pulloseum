...
- **덩굴 타입 42개체 전부 손그림 픽셀 격자 완료**(6 변이형 × 7속성). 6 변이형 실루엣(잎/독주머니/아가리/칼날·곤봉/뿔날개/무성잎)으로 모양=정체성, 속성은 리컬러만. 셀프테스트 0 fail. 다음 타입=다육(cactus).
...
- `green_vine`(normal=무성한 둥근 잎) + 기본 덩굴 6속성 — **속성마다 잎 모양을 다르게**(불=뾰족 불꽃잎·물=둥근 물방울잎·대지=넓은잎·바람=휜잎·번개=들쭉 번개잎·빙결=각진 결정잎)로 "색만 구분 금지" 준수. **6 변이형 템플릿 전부 확보.** 콘택트시트 실측·`__catalogSelfTest()` 0 fail.
