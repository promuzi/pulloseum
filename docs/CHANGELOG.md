...
### 2026-06-27 — 덩굴 속성별 효과 오버레이 (#3)
- 요청("각 속성에 맞게 변이별 특징 다르게 해석·속성 효과 섞어줘")으로 `renderElementFx` 추가 — 속성별 입자(불꽃·물방울·잎·바위·바람결·번개·결정)를 주변 + 변이형 핵심 부위(칼날끝·아가리·독주머니=`FORM_FX_FOCUS`)에 얹음. 같은 변이형도 속성마다 **모양**이 달라짐(색만 차이 아님). 셀프테스트 갱신·0 fail.
