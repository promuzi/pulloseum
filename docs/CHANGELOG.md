...
- **공통/변이 구분 확실화:** 기존엔 옅은 회색 텍스트 헤더뿐이라 공통 D-등급 줄이 변이형 헤더와 뭉개져 보였음. → 두 섹션(`mutDrawerHtml`)을 **색 패널**로 분리: 공통=시안(`#46b6ff`)·변이형=보라(`#c07bff`), 좌측 4px 액센트 바 + 틴트 배경 + 하단 구분선. 헤더에 아이콘·부제(모든 변이형 장착 / 해당 변이형만)·보유 수 배지 추가.
- **등급별 가로줄 정돈:** 등급 라벨 칩을 `align-self:stretch`로 그 등급 그룹 전체 높이에 맞춰 늘려 행 좌측 마커로 명확화, 등급 행 사이 점선 구분선. (그룹핑 로직 `mutGradeRowsHtml`은 기존 유지.)
- **검증:** preview 시안/보라 패널 경계 명확·grade S~D 가로줄 정렬 확인, `__catalogSelfTest()` 0 fail·콘솔 에러 없음. CSS만(`.drawer-sec`/`.drawer-sec-h`/`.dgrade-row`/`.dgrade-lab`) + 마크업 클래스(`sec-common`/`sec-variant`).
