...
### 2026-06-27 — 화분·식물 픽셀 크기 통일 + 식물 주변 효과 제거 (#3)
- 화분(cell5 큰 도트)과 식물(cell1.7 잔 도트)의 **픽셀 단위 불일치** 해소 — 식물 격자 `downscaleRows`(PIXEL_DOWNSCALE 0.66, 유효 cell≈2.6)로 해상도↓, 화분 `POT_PIXELS` 고해상 재작성(cell 2.8, ~20×15)+`potVisual` 자동정렬(중심 x60·바닥 y139). 둘 다 ~2.6~2.8 도트로 **중간 크기 통일**.
- 식물 주변 **속성 프릴(돌기) 제거**(renderElementFx 미사용) — 사용자 "이상한 효과 없애". 무기 변이형의 속성별 칼날은 유지.
- 셀프테스트 갱신(무기=속성별 칼날 상이·공통 변이=리컬러)·0 fail. 화분+식물 합성 렌더로 도트 크기 일치 실측.

### 2026-06-27 — 무기 변이형 속성별 칼날 재해석 (#3, 심화 1세트)
- 사용자 "한 세트만 먼저" → 무기 7종 칼날을 **속성별 다른 모양**으로: 불=불꽃날·빙결=결정날·번개=지그재그 번개날·물=물결날·풀=가시 톱날·바람=바람날·대지=돌몽둥이(공유 갑옷 줄기). `WEAPON_BLADES`+`WEAPON_STALK`로 7 무기 종 오버라이드. 변이 핵심부위(칼날)가 속성으로 재해석됨. 셀프테스트 0 fail. (확장 결정 대기 — 나머지 변이형도 같은 깊이로 할지)

### 2026-06-27 — 덩굴 속성별 형상 변형(외형이 속성으로) (#3)
- 요청("점 말고 외형이 불처럼 이글거리게·같은 변이도 속성마다 다르게 해석")으로 점 입자 폐기 → `ELEMENT_FRILL`(속성별 큰 돌기: 불꽃 혀·물갈퀴·겹잎·바위 갑옷·바람 날·번개 돌기·결정 가시)을 본체색+외곽선으로 몸체 둘레에 돋우고 본체 뒤에 그려 실루엣에서 자라나게. 같은 변이형도 속성마다 외형이 달라짐. 셀프테스트 0 fail. (변이 핵심부위 자체의 속성별 모양 재해석은 후속 42격자 작업.)
