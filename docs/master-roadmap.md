...
- [x] **식물 그림 내장 화분 제거 + 접지(2026-06-25)** — `composePlantSvg`의 어두운 내장 화분 삭제(식물만). 단독 표시(전투·모달·도감 등)는 `plantBaseY` 기준으로 viewBox crop해 식물이 바닥에 앉아 발밑 그림자와 연결. `noPot:true`=전체140(외부 화분용), 미지정=접지. 설계=[design](superpowers/specs/2026-06-25-plant-groundless-pot-design.md).
