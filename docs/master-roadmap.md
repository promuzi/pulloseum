...
| 3 | 도트 UI 적용 | 🟡 방향 확정 | 식물 1종 PNG 시범 → `SPRITE_OVERRIDES` | [pixel-art](pixel-art-ui-roadmap.md) |
...
- **메인 UI:** 상단 재화 헤더(레벨/크레딧/미네랄/설정), 프로필 배너, 식물 목록, 식물 관리창 3탭(강화/변이/스킬). 하단 탭은 **상점 / 탐사 / 식물·전투(중앙) / 식물양육 / 함선** 순서. 공용 팝업은 상·하단 헤더와 안전영역 사이에 스크롤 배치되며 탭 전환 시 닫힌다.
...
- **2026-06-23** — **UI 룩 = 흰 우주선 선체 타일 + 청록 네온**으로 전체 통일, 하단 탭마다 "다른 방" 분위기(탐사=어두운 관측 갑판/상점=단말기/전투=스테이지/양육=식물원/함선=정비 베이). 도트 PNG 교체와 호환되게 플랫·각진 프레임. (#11) **1단계 공통 크롬(상단바·하단탭·버튼) 완료**, 방별 분위기·내부 패널 화이트화는 후속. 세부 = `pixel-art-ui-roadmap.md`.
