...
| 9 | 구글 플레이 출시 | 🟢 **출시 스프린트 진행(2026-09-10~)** — UX P0-A 첫 세션 재배선 완료(잠금·배너 숨김, 첫 심기 1탭, 강화 모달 자동 오픈 폐지, 첫 전투 CTA, 3v3·미션 2승 해금) · **P0-B 가독성 완료**(갈무리 폰트 동봉·12px 하한·HUD 15px·히트 44/48px·눌림, 실측 12px 미만 0·44px 미만 0) · **P0-C 전투 피드백 완료**(1턴 6.0→4.4s·2배속 2.2s, 피격 플래시/히트스톱/넉백/흔들림, 데미지 24/32px, HP 이징·색 임계, 대기/공격 스텝 애니) · **P0-D 정리 완료**(IAP 카드 제거·함선 탭 잠금·fade-through·결과 장착 유도/패배 힌트) · **2026-09-11 피드백 반영**(함선 탭 잠금·HUD 숫자 15px·설정 알약·상대 다음 수 비공개로 되돌림) · 메인 스탯 8칸→핵심 4+성장 게이지 · 화면 캡처/실측 도구 `tools/shots/` | **🔖 다음 세션 진입점 = §4 #9 체크리스트**(Play 계정 확인 → 워크스루 → 로컬 번들·서명 → 스토어 자산 → 비공개 테스트, 병행 30종 스프라이트) | [ux-audit](superpowers/specs/2026-09-10-ux-audit-and-design.md) · [android](android-capacitor-wrapper.md) |
...
### 9. 구글 플레이 출시
**상태:** Capacitor 래퍼 + 디버그 APK 빌드 성공(현재 원격 URL 로드 방식). 절차 [android-capacitor-wrapper.md](android-capacitor-wrapper.md). **출시 스프린트(2026-09-10~):** 게임 쪽 UX P0-A~D와 사용자 피드백 1차 반영까지 완료 — 설계 [ux-audit](superpowers/specs/2026-09-10-ux-audit-and-design.md), 변경 내역 CHANGELOG 2026-09-10·11, 아트 [sprite-quality-research](sprite-quality-research.md). Play 개인 계정은 **비공개 테스트 12명×14일**을 통과해야 프로덕션 신청 가능 → 공개는 테스트 14일 + 심사 뒤.

**🔖 다음 세션 진입점 (위에서부터 순서대로):**
- [ ] **사용자 확인:** Play Console 개발자 계정 보유 여부($25 등록). 없으면 테스트 트랙 업로드 불가 — 가장 먼저 물을 것
- [ ] 인지적 워크스루 4문항 × 전 화면 + 5초 테스트 3화면(UX 설계 §4) → 걸린 곳 수정. 캡처 `node tools/shots/shoot.mjs --label <이름>`, 글자·버튼 크기 `node tools/shots/audit.mjs --label <이름>`
- [ ] **로컬 번들 전환** — 원격(GitHub Pages) URL 로드 설정을 빼고 웹 자산(`index.html`·`assets/`·`sw.js`)을 앱에 번들(`npx cap copy`) → 네이티브 세이브 브리지 안정성 확인
- [ ] 릴리스 키스토어 서명 + AAB 빌드 · Android 실기 확인: 뒤로가기·백그라운드 복귀 상태 보존·세이프 에어리어·Reduce Motion
- [ ] 스토어 자산: 아이콘 512·피처 그래픽 1024×500·폰 스크린샷(`shoot.mjs` 재활용)·짧은/긴 설명·개인정보 처리방침 URL·콘텐츠 등급 설문·데이터 보안 양식
- [ ] 비공개 테스트 트랙 업로드 + 테스터 12명 모집(14일 연속 유지) → RITE 방식으로 수정
- [ ] 에셋 라이선스 정리(#6 연동 — 폰트는 갈무리 OFL 동봉 완료), 버전 정책(versionCode/Name)
- [ ] **병행(아트 #3):** 나머지 30종 도트 레시피(`tools/pixel/sprites-data.js`, 속성별 7종) → 생장 단계별 그림 → 대기 프레임
- [ ] **알려진 이슈:** 셀프테스트 기존 실패 1건 "team: loadout 6 + team state migration…"(스프린트 이전부터) · 메인 왼쪽 변이 알약의 긴 이름 잘림(예: "포식·흡혈")
...
- **2026-09-11** — **(전투 UX — 상대 다음 수 비공개 확정)** 사용자: "그런 거 하지 마, 나중에 PvP 할 거니까". P0-C에서 넣은 매 턴 의도 칩(종류·예상 피해)은 사람 상대에겐 줄 수 없는 정보라 **폐기·재도입 금지**. 봇 정보는 2026-07-02 방식(3코스트+ 큰 기술 55% ⚠️·종류 숨김 + 성향 표시)만. 셀프테스트 가드로 고정. (#10·#8)
- **2026-09-11** — **(UI 피드백)** 함선 탭 = **보이되 잠금**(나중에 키울 기능, 탭하면 준비 중 안내) · 상단 HUD 숫자 **15px**(24px는 "너무 큼", 갈무리 선명 크기 12·15·24px) · 설정 버튼은 다른 알약과 같은 32px + 투명 44px 히트 영역. (#4·#9)
- **2026-09-10** — **(#9 출시 스프린트 착수)** 1주일 출시 목표로 풀로세움 선정. Play 개인 계정 = 비공개 테스트 12명×14일 선행. 아트 = 레트로 도트 48×56 직접 작화(렌더러 v2, 5종 반입). UX P0 구현: 첫 세션(첫 심기만 1탭·강화창 자동 오픈 폐지·3v3/미션은 1v1 2승 뒤) / 가독성(갈무리 동봉·12px 하한·히트 44) / 전투 피드백(`hitFx`·2배속·1턴 6.0→4.4s) / 정리(IAP 카드 제거·방 전환 fade-through·결과 장착 유도). 메인 스탯 8칸 → 하단 핵심 4 + 성장 게이지. (#9·#3·#10)
