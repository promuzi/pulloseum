...
| **[superpowers/specs/2026-06-26-spore-variant-rework-dual-variant-mushrooms-design.md](superpowers/specs/2026-06-26-spore-variant-rework-dual-variant-mushrooms-design.md)** | 포자 재정의+이중 변이 버섯(🟡브레인 중·방향만 합의·재개 예정) — `baseVariants:['spore',X]`, 융합 전용 메커니즘 폐기(포자=무-스킬 패시브 아우라라 공존으로 충분), 필요 구현=`cardFitsForm`+`variantSkillsOf` 둘 다 `base_variants` 전체로 확장, 포자=매턴 발산 보장+효과별 확률. 미정=발산 범위(A포자카드만/B변이카드전부)·두번째변이 종류·마릿수 | **#1 버섯 확장 — 재개 진입점** |
