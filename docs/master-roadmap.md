...
| # | 방향 | 상태 | 다음 한 걸음 | 관련 문서 |
|---|------|------|-------------|----------|
| 1 | 식물 종류 확장 | 🟢 base 35 + 변이 140 + 탐사 분포 / 🟢 **컨셉+스킬확장: P0 엔진 5종+인프라 + 140 비버섯 변이 단계별 컨셉 스킬(10 손작업+130 생성기) 완료** | **버섯 7·base 28 + 큐레이션(어색 이름/획일 개체)** (§4-1-A) | [species](species-system-guide.md) · [concept-skill](superpowers/specs/2026-06-26-individual-concept-skill-expansion-design.md) · [plan](superpowers/plans/2026-06-26-individual-concept-skill-expansion.md) |
...
| 11 | 시작화면 수정 | 🟡 암실 인트로 구현 | 검은 암실+펜던트 전등(낙하·스윙·지지직)→터치 시 CRT 점등으로 메인 연결(`#introOverlay`/`startIntro`/`powerOnIntro`). 타이틀 폐지. 후속: 사운드·전등 줄당김 인터랙션 | — |
...
### 5. 애니메이션 추가
**상태:** VS 인트로·진화 연출·전투 판정 연출·**시작 암실 인트로** 일부 구현.
- [x] 도입부/전주(타이틀→게임) 전환 연출 — 암실 인트로(전등 낙하·스윙·지지직→터치 CRT 점등). 상세 #11.
