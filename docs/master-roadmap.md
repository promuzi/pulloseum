...
| 3 | 도트 UI 적용 | 🟢 홀로그램 오버레이 + **개체 손그림 픽셀 격자(redraw) 인프라 + 덩굴 파일럿 2종(2026-06-27)** + 런타임 픽셀화 폴백 공존 | 덩굴 나머지 40종 → 타입별 묶음(다육→화초→목본→버섯) | [redraw spec](superpowers/specs/2026-06-27-pixel-sprite-individual-redraw-design.md) · [unification spec](superpowers/specs/2026-06-26-pixel-art-unification-design.md) · [pixel-art](pixel-art-ui-roadmap.md) |
...
> 🔖 **로드맵 구체화 이어가기 →** 미완 애매 영역(탐사선 강화UI·화분 구체화·함선 스토리/길드·아키타입 효과·서버 묶음)의 진행 상태와 1차 제안은 **[§4.6 재개 진입점](#46-로드맵-구체화--미완-애매-영역--재개-진입점-2026-06-26)**. #15 미션 모드는 설계 완료(다음=writing-plans).
...
**✅ 완료한 영역:**
- **#15 전투-미션/스토리 모드** — 설계 확정·박제([mission spec](superpowers/specs/2026-06-26-mission-story-mode-design.md)), 표·§4-15·결정로그 등록 완료. **다음 = writing-plans로 구현 계획**(미착수).
...
4. **고유스킬 효과 다양화 (아키타입 효과)** *(#1-A·#6 묶음)*
   - **가치:** 14 아키타입이 이름·DoT종류만 다르고 **실제 효과는 동일**("광폭과 도사가 똑같이 때림").
   - **1차 제안:** 아키타입별 **효과 경향 modifier**(도사=치명집중, 책략=디버프 포진, 수호=방어버프↑, 탐식=흡혈↑ 등)를 고유스킬에 실제 반영. §4.5-A 밸런스 5종과 묶어 "밸런스/스킬 다듬기" 트랙.
   - **확정 필요:** 14 아키타입 각 효과 modifier 정의, 엔진 반영 지점.
...
- **2026-06-27** — **(#3 도트 UI — 개체 손그림 픽셀 격자(redraw) 인프라 + 덩굴 파일럿)** "모든 개체 외형" 방향을 두 접근 충돌(런타임 픽셀화 vs 개체별 손그림 격자) 중 **사용자 결정=redraw(개체별 고유 격자, 색만 구분 금지·하드 원칙) + 런타임 픽셀화는 폴백 공존**으로 확정. 구현: `STRUCT_PALETTE`(구조 고정색)+`elementBodyPalette`(속성 본체색)+`SPECIES_PIXELS` 레지스트리+`renderSpeciesPixels`+`composePlantSvg` 분기(등록 종·`gi>=3`→픽셀 경로, `featuresSvg`/`markingSvg`/variant필터 스킵, `data-pixelgrid` 표식). 런타임 픽셀화 사후패스는 격자를 **재픽셀화 스킵**(공존). 덩굴 파일럿 `venom_weed`/`thorn` 2종(실루엣 구분·속성 리컬러 불변 실측). `__catalogSelfTest()` 0 fail(픽셀 경로/폴백·리컬러 불변 케이스 추가). 다음=덩굴 양산→타입별 묶음. 설계=[redraw spec](superpowers/specs/2026-06-27-pixel-sprite-individual-redraw-design.md). (#3)
- **2026-06-27** — **(#3 도트 UI — 전 개체 런타임 픽셀화 구현, unification 1·2단계 / redraw 폴백으로 격하)** "모든 개체 외형 바꾸기" 요청 → 손그림 1,000장+ 대신 설계 #16 D2(엔진 런타임 픽셀화)로 **자산 0개 전 개체 도트화**. `--px:3px` 규정 + `PIXEL_PALETTE`(7속성 램프+중립 ≈24색) + `pixelizeSvg`(SVG→캔버스 다운샘플→팔레트 양자화→`toDataURL`, 외형 해시 캐시) + `MutationObserver` 사후 패스(`.ss-plant`/`.plant-layer`만 `<img>` 교체, 화분 제외, `data-px` 루프차단). 비동기 굽기를 렌더 경로 밖으로 분리해 셀프테스트·dex 동기성 무영향. Chromium 검증: 셀프테스트 0 fail·변이 hue필터/오염 없음·전 식물 픽셀화·화분 0·dex 미실행. 후속=3단계 UI 토큰화·4단계 AI그림·5모션·6사운드. 설계=[unification spec](superpowers/specs/2026-06-26-pixel-art-unification-design.md). (#3·#16)
