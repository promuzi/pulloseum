...
| 9 | 구글 플레이 출시 | 🟢 **출시 스프린트 진행(2026-09-10~)** — UX P0-A 첫 세션 재배선 완료(잠금·배너 숨김, 첫 심기 1탭, 강화 모달 자동 오픈 폐지, 첫 전투 CTA, 3v3·미션 2승 해금) · **P0-B 가독성 완료**(갈무리 폰트 동봉·12px 하한·HUD 24px·히트 44/48px·눌림, 실측 12px 미만 0·44px 미만 0) · **P0-C 전투 피드백 완료**(1턴 6.0→4.4s·2배속 2.2s, 피격 플래시/히트스톱/넉백/흔들림, 데미지 24/32px, HP 이징·색 임계, 상대 의도 매 턴, 대기/공격 스텝 애니) · **P0-D 정리 완료**(IAP 카드 제거·함선 탭 숨김·fade-through·결과 장착 유도/패배 힌트) · 메인 스탯 8칸→핵심 4+성장 게이지 · 화면 캡처/실측 도구 `tools/shots/` | 인지적 워크스루·실기(Android) 확인 → 로컬 번들·서명 → 비공개 테스트(12명×14일). 병행: 나머지 30종 스프라이트 | [ux-audit](superpowers/specs/2026-09-10-ux-audit-and-design.md) · [android](android-capacitor-wrapper.md) |
