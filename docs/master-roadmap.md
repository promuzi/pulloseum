...
| 9 | 구글 플레이 출시 | 🟢 **출시 스프린트 진행(2026-09-10~)** — UX P0-A 첫 세션 재배선 완료(잠금·배너 숨김, 첫 심기 1탭, 강화 모달 자동 오픈 폐지, 첫 전투 CTA, 3v3·미션 2승 해금) · **P0-B 가독성 완료**(갈무리 폰트 동봉·12px 하한·HUD 24px·히트 44/48px·눌림, 실측 12px 미만 0·44px 미만 0) · 화면 캡처/실측 도구 `tools/shots/` | P0-C 전투 피드백 → P0-D 정리 → 로컬 번들·서명 → 비공개 테스트(12명×14일) | [ux-audit](superpowers/specs/2026-09-10-ux-audit-and-design.md) · [android](android-capacitor-wrapper.md) |
...
| **15** | **전투-미션/스토리 모드** ⭐신규 | ✅ **1차 구현 완료(2026-07-02)** — 캠페인 3종(볼카르★2·네레이돈★3·전 챔피언★5 선행게이트)·고정 적(난수 0=결정론)·보스 기믹(작열 오라/선제 방어막/격노 활성, 가시반사/재생은 데이터만 꽂으면 됨)·보상종 `released:false` 우회 지급+칭호+개봉연출·스토리 텍스트. 진입=토너먼트 바 "🚩 미션 캠페인" 배너. E2E preview 검증 | 캠페인 데이터 추가 저작 · 함선 NPC 해금 연동(#4) · 프로필 칭호 노출 | [mission spec](superpowers/specs/2026-06-26-mission-story-mode-design.md) · 본문 [§4-15](#15-전투-미션스토리-모드-신규) |
| **16** | **탐사선 강화 UI(격납고 개조실)** | ✅ **구현 완료(2026-06-26)** — 라이브 `exHangarHtml`을 중앙 탐사선 SVG+사방 4콜아웃 연결선 다이어그램으로 + 강화 레벨(Lv) 표시(탐사선·식물 공용) | (후속) 죽은 연료/레거시 코드 정리·SVG 외형 다듬기 | [plan](superpowers/plans/2026-06-26-explorer-ship-upgrade-ui-redesign.md) · CHANGELOG 2026-06-26 |
| **17** | **전투 코어 재설계** ⭐재미 재설계 1부 | 🟡 **1v1 주력으로 방향 확정(2026-07-02 플레이테스트 2회 반영)** — 최종 형태: **1v1 메인**(6스킬+변이 바·개체 랭크) + 봇 아키타입 성향 tell+큰기술 ⚠️예고 + 에너지 커브(시작2·+1/턴·최대8) + 의도 수집/해소 엔진. 손패4 덱 순환·8장 로드아웃은 플레이테스트로 폐지. 3v3(교체 심리전·재배사 랭크)·미션 보스+호위전은 **보조 콘텐츠 존치** | **스킬 구동 심화 브레인**(차지·스택 가시화·연계 보너스) · 밸런스 체감 튜닝 · 2부 메타루프 | [spec](superpowers/specs/2026-07-02-battle-core-3v3-redesign-design.md) · 결정로그 2026-07-02×3 |

상태 기호: 🔲 미착수 · 🟡 진행/설계 중 · 🔵 설계 완료·구현 대기 · ✅ 완료 · ❄️ 보류

> 🔖 **로드맵 구체화 이어가기 →** 미완 애매 영역(화분 구체화·함선 스토리/길드·서버 묶음)의 진행 상태와 1차 제안은 **[§4.6 재개 진입점](#46-로드맵-구체화--미완-애매-영역--재개-진입점-2026-06-26)**. #15 미션 모드·아키타입 효과는 **구현 완료(2026-07-02)**.
...
| **[superpowers/specs/2026-06-26-spore-variant-rework-dual-variant-mushrooms-design.md](superpowers/specs/2026-06-26-spore-variant-rework-dual-variant-mushrooms-design.md)** · [plan](superpowers/plans/2026-06-26-dual-variant-mushrooms.md) | 포자 재정의+이중 변이 버섯(✅구현완료 2026-06-26) — `baseVariants:['spore',X]` 28종(4변이×7속성), `cardFitsForm`+`variantSkillsOf`를 `base_variants` 전체로 확장(공존, 융합 메커니즘 없음), 포자=매턴 발산 항상표시+효과별 확률, 외형 변이색 액센트, `MUTANT_SIGNATURES` 분포. 생성기 `scripts/gen-dual-mushrooms.js`. 셀프테스트 0 fail | **#1 버섯 확장 — 완료** |
...
| **[superpowers/specs/2026-07-02-battle-core-3v3-redesign-design.md](superpowers/specs/2026-07-02-battle-core-3v3-redesign-design.md)** | **전투 코어 3v3 재설계(🟡 승인 대기)** — 팀3·교체 심리전(턴 소모)·동시 비밀 선택+봇 성향 tell+큰기술 예고·덱8/손패4·에너지 커브. 재미 재설계 3부작의 1부(2부 메타루프·3부 UI 전면은 후속 브레인). **재개 시 이 문서부터** | **재미 재설계 — 진행 중** |
...
- [x] **고유스킬 효과 다양화 (2026-07-02)** — 14 아키타입 전부 `fx` 실효과 부여(`ARCHETYPES[*].fx` + `__applyArchFx`, 멱등 가드): 광폭=위력+8%·치명+5% / 도사=치명+15% / 교활=적중 디버프+증폭 / 우직·수호=방어 버프(+증폭) / 광신=위력+12% / 냉혹=관통30%+치명 / 야성=위력+속도버프 / 책략=방깎+디버프 증폭 / 은둔=회복 라이더 / 폭군=공깎 / 탐식=흡혈20% / 망령=에너지 흡수+치명 / 맹장=관통25%+공버프. 스킬 desc에 【아키타입】 표기 자동 추가. 셀프테스트(적용·멱등·14종 실사용·스팟체크) 0 fail.
...
> ✅ **1차 구현 완료(2026-07-02).** 아래 확정 골격 그대로 구현됨(캠페인 3종·기믹 5종 정의 중 3종 콘텐츠 사용·E2E preview 검증). 구현 상세 = §5 결정 로그 2026-07-02. **잔여:** 캠페인 추가 저작(데이터만 추가하면 확장)·함선 NPC 해금(#4)·프로필 칭호 노출. 진입 UI는 세그먼트 탭 대신 **토너먼트 바 전폭 "🚩 미션 캠페인" 배너**로 구현(같은 위치·한 탭 절약).
...
## 4.6 로드맵 구체화 — 미완 애매 영역 (🔖 재개 진입점, 2026-06-26)

> **다른 세션/기기에서 "로드맵 구체화"를 이어갈 때 여기부터.** 사용자가 제시한 7대 작업 방향을 기존 로드맵에 재정비하던 중, **신규 방향과 애매 영역들의 가치·동작을 하나씩 브레인스토밍으로 구체화**하는 작업을 진행 중이다. 아래는 그 진행 상태와 **각 영역의 1차 구체화 제안(=다음 세션의 출발선)** 이다. 각 영역을 다룰 땐 `superpowers:brainstorming`으로 제안을 다듬어 확정 → 스펙 박제 → 표/§4 본문/결정로그 등록 → (필요 시) writing-plans.

**✅ 완료한 영역:**
- **#15 전투-미션/스토리 모드** — 설계 확정·박제([mission spec](superpowers/specs/2026-06-26-mission-story-mode-design.md)) → **✅ 1차 구현 완료(2026-07-02, §1 표 #15·결정로그 참조).**
- **고유스킬 효과 다양화(아키타입 효과)** — **✅ 구현 완료(2026-07-02).** 14 아키타입 전부 `fx` 실효과 반영(§4-1-A 체크박스·결정로그 참조). "광폭과 도사가 똑같이 때림" 해소.

**🔜 미완 애매 영역 (가치는 잡혔으나 동작 미확정 — 구체화 대기):**

1. **탐사선 강화 UI** *(✅ 구현 완료 2026-06-26 — §1 표 #16 등록됨)*
   - **가치:** 탐사선 4스탯 강화가 묻혀 있음 → 식물 강화처럼 **눈에 보이는 성장 축**으로 표면화.
   - **구현:** 라이브 격납고 `exHangarHtml`을 **중앙 탐사선 SVG + 사방 4콜아웃(상 탐사장치·하 연료탱크·좌 내구성·우 채집기) + 부품 위치로 점선 연결선**(`drawShipConnectors`)으로 재설계. 강화 비용=크레딧, 기존 `SHIP_UPGRADES`/`upgradeShipStat` 단일 경로 재사용(신규 강화 경로 없음·표면화만). **강화 레벨(Lv) 표시**(탐사선 콜아웃+식물 강화 카드 공용 `upgradeLevelText`). ⚠️ 계획서가 지목한 `renderShipModRoom` 등은 죽은 코드였고(라이브=`ex*` 재작성본) Option A로 라이브 함수에 재지정. 상세=CHANGELOG 2026-06-26.
   - **후속:** 죽은 연료/레거시 코드(`executeExploration`·구 `renderExploration` 3종·`renderShipModRoom`) 정리, 탐사선 SVG 외형 다듬기.

2. **화분 기능 구체화** *(신규, 표 미등록 — 다룰 때 #17로 등록 예정)*
   - **가치:** 화분이 "충전속도/확률 숫자만 다른 5종"이라 수집 동기 약함 → 화분마다 **'성격'** 부여.
   - **1차 제안:** ① 화분별 **고유 특수효과**(예: 유리=특정 등급 편향, 크리스탈=물 트리클 가속, 황금=즉시수확 미네랄 할인) ② **획득/경제 명확화**(상점/탐사/미션 어디서·가격·해금조건). 시각 도트(#3)·동시양육 슬롯은 하위 열린 항목.
   - **확정 필요:** 화분별 고유효과 구체값, 획득 경로 매핑.

3. **함선 — 스토리·길드** *(#4 하위 구체화)*
   - **가치:** 함선이 "기능 진입 허브"일 뿐 서사·소속감 없음.
   - **1차 제안:** ① **스토리 = 함선 NPC 대화가 미션 캠페인(#15) 해금 트리거** → 함선이 스토리 허브. ② **길드 가입은 #8 서버 전제** → 오프라인 단계엔 *로컬 더미 길드*(양육속도/탐사보너스 소폭)만, 실 길드는 서버 도달 후.
   - **확정 필요:** NPC/대화 데이터 구조, 더미 길드 버프 범위.

4. **고유스킬 효과 다양화 (아키타입 효과)** *(✅ 구현 완료 2026-07-02 — §4-1-A 체크박스 참조)*
   - **구현:** `ARCHETYPES[*].fx` + `__applyArchFx`(멱등 가드) — 14종 전부 실효과(치명/관통/흡혈/에너지흡수/버프·디버프 부여·증폭/위력/회복) 부여, desc에 【아키타입】 자동 표기. 반영 지점 = `applyVariantIdentity` 리테마 루프.
   - **후속:** 밸런스 체감 튜닝(과/소 보정)·개체별 fx 오버라이드(큐레이션).

5. **서버 묶음** *(#8·#9 통합 마일스톤)* — 🔵 **아키텍처 확정(2026-06-26)**
   - **확정:** **풀 서버 권위**(진행=서버 발급, 전투=서버 재검증)로 치트(조작 만렙→PvP)를 **출시 전제로 원천봉쇄.** 백엔드=Firebase(Auth+Firestore)+Cloud Functions. 익명기본+구글연동, 안드 네이티브 로그인. 온라인 필수. 순서=기둥1(서버권위 계정·세이브 포함)→출시→기둥2(전투 재검증)→결제→PvP. 전체 = [server-authority spec](superpowers/specs/2026-06-26-server-authority-anticheat-design.md).
   - **지금부터 준수:** 전투 시뮬 **결정론 규율**(CLAUDE.md 하드 지침) — 신규 전투/성장 코드는 시드 단일 `rng()`·순수 계산. 미래 공유모듈 추출 비용 절감.
   - **연기:** 엔드포인트·DB 스키마 등 상세 스펙은 실제 서버 작업 직전(launch가 멀어 지금 쓰면 썩음).

**보류된 결정(미승인):** 신규 3방향(#15·탐사선강화·화분)의 **표 우선순위 정렬안**(UI/도트→함선→탐사선강화→미션→서버→밸런스→화분)은 제안만 하고 미확정. #16·#17 표 등록은 각 영역 구체화 시 함께.
...
- **2026-07-02** — **(#17 방향 확정: 1v1 주력 회귀 — 3v3=보조 리그)** 사용자 판단: 개체당 육성 정성(변이·강화·스킬)이 게임의 축이라 **3v3 주력은 정성 3배 분산의 구조적 모순** → **1v1 주력 확정**, `MAX_LOADOUT` 8→6 + 변이 스킬 별도 바(`#cardPred`) 복원, 메인 버튼=1v1 토너먼트. **존치:** 봇 성향 AI+예고(1v1 "읽는 재미"=이번 공사 최대 수확)·에너지 커브·의도 수집/해소 엔진·팀 인프라(미션 보스+호위전·3v3 보조 리그·재배사 랭크 — 유지비 0). **다음 재미 탐색은 스킬 구동 심화(차지/스택 가시화/연계 보너스)·2부 메타루프에서.** (#17)
- **2026-07-02** — **(#17 플레이테스트 번복: 손패4 덱 순환 폐지)** 사용자 실플레이 피드백 — 랜덤 손패가 스택/콤보 셋업(비늘·브레스·태그 콤보)을 차단해 턴제와 상성 나쁨 + 숨 고르기 무의미(0코 기본기 존재). **스킬 8장 상시 오픈으로 회귀**, 숨 고르기=사용 가능 스킬 0일 때만 폴백, 긴장은 에너지 커브가 담당. `makeDeckState/deckPlay/deckNext`·다음 카드 UI 제거. 부수 버그: 토너먼트 바 위임 리스너가 모든 `.tcell`에 참가경기 모달을 열던 것 → `[data-matches]` 한정. **스킬 구동 심화(차지/스택 강화 등)는 열린 과제로 유지.** (#17)
- **2026-07-02** — **(#17 전투 코어 3v3 재설계 — ✅ 1차 구현 완료, 재미 재설계 1부)** 브레인스토밍으로 5개 뿌리 결정 확정(비전 B 팀3+심리전 / 하이브리드 정보구조 / 교체=턴 소모 / 덱8·손패4+에너지 커브 / **랭크 분리 두 리그**) 후 같은 날 Task 1~10 구현. **아키텍처:** `B.p/B.e`=활성 유닛 별칭(기존 호출부 무접촉)+`pTeam/eTeam/pActive`, 에너지=**팀 공유 풀 accessor**(`makeEnergyPool`/`attachEnergyPool` — 기존 `a.energy±=` 무수정 통과), 덱=`makeDeckState/deckPlay/deckNext`(rng 셔플=결정론), 턴=`submitIntent`→`aiPickIntent`→`resolveTurn`(교체 우선→속도순 해소→기절 무료교대 `promptFreeSwitch`→팀 전멸 종료). **봇:** `AI_TEMPER` 14 아키타입 성향 가중(+`buildEnemy`가 base 종에도 `archetypeOf` 배정)·불리 상성 교체 판단·3코+ 큰 기술 ⚠️예고(`updateEnemyForecast`, 예고한 수는 지킴). **두 리그:** 1v1=개체 랭크(기존 계승, `state.tournament`) / 3v3=재배사 랭크(`team_rankPoints/team_rankTier/team_tournament`+`endTeamMatch`·보상 1.25×), 토너먼트 바에 3v3 리그 행. **미션:** 스테이지 `enemies[]`(1~3) 스키마+하위호환(`missionEnemiesAt`), 캠페인 3종 재저작(보스+호위·챔피언 팀전). **UI:** 팀 바(상성 ▲▼·기절 pick 모드)·손패4+다음 카드·숨 고르기(⚡+1 폴백)·성향 표시. 구 `aiPickSkill`/`#cardPred` 제거, 결정론 가드 테스트를 신 함수들로 확장. 셀프테스트 신규 7종 포함 **0 fail** · preview E2E(1v1/3v3/미션 팀전/교체/예고). 스펙=[battle-core-3v3](superpowers/specs/2026-07-02-battle-core-3v3-redesign-design.md) · 플랜=[plan](superpowers/plans/2026-07-02-battle-core-3v3-redesign.md). **다음=2부 메타루프 브레인.** (#17)
- **2026-07-02** — **(#15 전투-미션/스토리 모드 — ✅ 1차 구현 완료)** 설계(2026-06-26 spec) 그대로 구현. **데이터:** `CAMPAIGNS` 3종(볼카르 화산 군락★2 3+보스 / 네레이돈 심해 원정★3 4+보스 / 전 챔피언의 시험★5 3+보스·`unlock:{clear:'nereidon_deep'}` 선행게이트) + `MISSION_GIMMICKS` 5종 + `state.mission_progress`/`mission_titles`(`normalizeState` 마이그레이션). **적:** `buildMissionEnemy` — 저작 고정 사양(스케일 봇 아님)·**난수 0=결정론 규율 준수**(같은 스테이지=항상 같은 적), 스케일=생장×★×부스트, 봇과 동일 `makeCombatant` 재사용. **보스 기믹:** 작열 오라(=`tickStatuses` 신규 aura 블록·화상 자동부여), 선제 방어막(=기존 `barrier`), 격노(=아드레날린 `turnHooks.threshold` 재사용), 가시반사/재생(=기존 패시브, 데이터만 꽂으면 활성). **전투 배선:** `startMatch`에서 `makePlayerCombatant`/`bootBattleUI` 추출(토너먼트/미션 공용) + `startMissionBattle`/`endMissionMatch`(`endMatch` 최상단 분기, 토너먼트 상태 무접촉). **보상:** 첫클리어=크레딧/미네랄·재도전=20% 소액, 캠페인 완료=**보상종 시드 확정 지급**(`grantMissionSpecies` — `released:false` 우회 명시 경로, 봇/일반 풀 불변)+칭호+`openRewardReveal` 개봉연출+outro. **UI:** 토너먼트 바 전폭 "🚩 미션 캠페인" 배너(n/N 클리어) → `#missionModal` 캠페인 목록(★·권장·진행·잠금)→내부 스테이지 사다리(등장대사·기믹 표시·스토리 박스). 셀프테스트 3종(데이터 정합·결정론+기믹 배선·released 우회) 추가 0 fail · **preview E2E 실측**(스테이지 클리어→진행/보상, 보스 클리어→종자+칭호+연출, 작열 오라 실발동, 선행게이트 잠금). **후속:** 캠페인 추가 저작·함선 NPC 해금(#4)·프로필 칭호 노출. (#15)
- **2026-07-02** — **(#1-A 아키타입 실효과(fx) — "이름만 다른 문제" 해소)** 14 아키타입이 이름·DoT만 다르고 효과 동일하던 것(§4.6-4)을 구현으로 해소. `ARCHETYPES[*].fx` 신설 + `__applyArchFx()`가 `applyVariantIdentity` 리테마 루프에서 `ind.<key>.g/m/e`에 실효과 부여(additive라 `sk.__archFx` 가드로 멱등): 광폭 위력+8%·치명+5% / 도사 치명+15% / 교활 적중디버프+증폭 / 우직 방버프 / 수호 방버프+증폭 / 광신 위력+12% / 냉혹 관통30%+치명8% / 야성 위력+5%·속버프 / 책략 방깎+디버프증폭1.3 / 은둔 회복6% / 폭군 공깎 / 탐식 흡혈≥20% / 망령 ⚡1흡수+치명 / 맹장 관통25%+공버프. **엔진 제약 준수**(버프/디버프 스킬당 1개 → "없으면 부여·있으면 증폭"), desc에 【아키타입】 라이더 자동 표기(도감·상세 자동 반영). 셀프테스트(전 대상 적용·14종 실사용·멱등·스팟체크) 0 fail. (#1-A)
- **2026-07-02** — **(밸런스 §8 확정 이슈 3건 수정)** ① **BST 과벌점 완화:** `ELEMENT_STATS` 불 hp40→46·atk14→15(+7), 바람 hp40→44·def5→7(+6) → 종간 격차 ~42→~35(컨셉 유지). ② **무기 등급 위력 완충:** `gradeCardSkill`이 noEnergy(무기) 스킬 위력만 `1+(mult−1)×0.6`으로 스케일 — S 내려찍기 409→331, D등급은 소폭 상향(0.8→0.88). ③ **단일/광역 명중 방향 반전:** 1:1 게임인데 광역+10·단일−7이던 `applySkill` accMod를 단일+7·광역+0으로 수정(바람 단일 속성기 손해 해소, 전 스킬 공통). balance-sheet §8 2·3·4 취소선 처리 + §1-2 표 갱신. 셀프테스트 0 fail. (#1·밸런스)
- **2026-07-02** — **(폴리시) 한국어 조사 유틸 `josa(word, pair)` 신설** — 받침 유무로 을/를·이/가·은/는·과/와·으로/로 자동 선택(비한글 끝 병기 폴백, ㄹ받침 '로' 처리). "감사용를 심었습니다"(심기 토스트)·"방어이 증가하였다!"(전투 버프/디버프 문구) 등 8곳 교체, 셀프테스트 추가. 새 동적 문구는 `josa()` 사용할 것. (폴리시)
...
- **2026-06-26** — **(#8 서버 — 아키텍처 확정: 풀 서버 권위 + 치트방지)** 서버 묶음 방향을 브레인스토밍으로 확정. **위협 정의:** "재화·레벨 조작으로 만렙 식물 만들어 PvP 진입" = 막아야 할 핵심 치트. **클라 권위+blob 백업 안은 폐기** — PvP를 나중에 붙여도 그 전에 만든 치트 계정이 그대로 진입하므로 **출시 전제로 원천봉쇄**해야 함. **확정:** ① **풀 서버 권위** = 진행(경험치·재화·식물)을 **서버만 발급**(기둥1, "치트 만렙" 성립 불가·크로스기기 세이브 자동 포함) + **전투를 서버가 재검증**(기둥2, 결정론 공유 시뮬). ② 백엔드=Firebase(Auth+Firestore)+Cloud Functions. ③ 익명 기본+구글 연동, 안드 네이티브 로그인 플러그인. ④ 온라인 필수(서버 권위의 자연 귀결). ⑤ 결제=entitlements+구글 purchaseToken 서버검증. **가장 큰 비용=전투엔진을 결정론 순수 공유모듈로 추출**(JS라 브라우저+Node 공유 가능). **지금부터 결정론 규율을 CLAUDE.md 하드 지침화**(시드 단일 `rng()`·순수 계산)해 추출 비용 절감. **엔드포인트/DB 상세는 서버 작업 직전으로 연기**(launch 멀어 지금 쓰면 썩음). 설계=[server-authority spec](superpowers/specs/2026-06-26-server-authority-anticheat-design.md). (#8·#9)
- **2026-06-26** — **(로드맵 구체화 세션 — 재개 진입점 박제, §4.6)** 사용자 7대 작업 방향을 기존 로드맵에 재정비하며 신규 방향·애매 영역의 가치·동작을 브레인스토밍으로 구체화 착수. **#15 미션 모드는 완료(스펙 박제·등록)**, 나머지 5개 애매 영역(탐사선 강화UI·화분 구체화·함선 스토리/길드·아키타입 효과·서버 묶음)은 **1차 제안만 박제하고 보류** — 다른 세션/기기가 이어받도록 §4.6 "재개 진입점"에 진행 상태+각 영역 출발선 제안 정리. #16·#17 표 등록과 우선순위 정렬안은 각 영역 구체화 시 함께. (로드맵 운영)
