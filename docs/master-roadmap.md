...
1. **탐사선 강화 UI** *(신규, 표 미등록 — 다룰 때 #16으로 등록 예정)*
   - **가치:** 탐사선 4스탯 강화가 묻혀 있음 → 식물 강화처럼 **눈에 보이는 성장 축**으로 표면화.
   - **1차 제안:** 식물 강화창 레이아웃 재사용 — 가운데 탐사선 SVG + 좌우 4스탯 버튼(연료=궤도해금·내구=탐사횟수·채집기=수확량·탐사장치=희귀종확률). 비용=크레딧/미네랄. **신규 강화 경로 없음, 표면화만.**
   - **확정 필요:** 강화 곡선/상한, 탐사선 SVG 외형, 버튼 배치.
