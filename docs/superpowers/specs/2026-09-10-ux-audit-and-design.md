...
| 적 의도는 전부 보여준다. "멋진 아이디어는 명확성을 위해 매번 희생" | Into the Breach 개발자, 2018 | 3코스트 이상만 예고 → **모든 턴 상대 의도 아이콘**(공격/방어/버프 + 예상 위력)으로 확대 |
...
| 2~4 | 첫 전투, 첫 상성 피드백 | 강화 모달 자동 오픈 폐지. 심기 직후 CTA = "첫 전투". 2턴째에 상대 의도가 보이는 상태에서 상성 때문에 스킬을 바꾸는 결정 1회. 결과는 플래시·숫자·문구·사운드 동시 |
...
### 7-9. 전투 (§6 타이밍 표와 함께)
- 무대: 상대 위·나 아래 대각 배치 유지. **무대 높이를 줄여** 빈 어둠을 없애고(스프라이트 96px 기준 무대 ≈ 화면 45%), 남는 높이는 스킬 바와 상대 의도 카드에.
- 상대 의도: 매 턴 상대 카드 위에 **의도 아이콘 + 예상 위력**(Into the Breach·StS 방식). 현재 3코스트 이상만 예고 → 전부.
...
| 3 | **P0-C 전투 피드백**: 판정 타이밍 표 적용(sleep 재배분) / 흰 플래시·넉백·히트스톱 / 데미지 숫자 크기·색 / HP바 이징·색 임계 / 2× 토글 / 상대 의도 매 턴 표시 / "에너지" 줄바꿈 | §6, §7-9 |
