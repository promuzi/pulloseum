...
## 3.5 강화 레벨 표시 (식물 강화창 + 탐사선 개조실 공통)

현재 두 강화창 모두 **몇 단계 강화했는지 숫자가 안 보인다.** 각 강화 카드/박스에 **레벨 배지**를 추가한다.

- **규칙:** 표시 레벨 = `강화 횟수 + 1` (시작 = **Lv.1**, 한 번 강화할 때마다 +1). 데이터는 이미 존재 — 신규 필드 불필요.
  - 식물: `p.up[스탯]`(0부터) → 카드별 `Lv.(p.up[k] + 1)`. (스탯 8종: hp/atk/def/spd/acc/crit/elem/energy)
  - 탐사선: `ship.upgrade_levels[트랙]`(0부터) → 박스별 `Lv.(level + 1)`. (트랙 4종)
- **단위:** **스탯/트랙별 레벨**(각 카드가 독립 Lv 표시). ⚠️ 검토 포인트 — 사용자가 "식물/탐사선 전체 1개 레벨"을 원하면 변경(§7).
- **최대치:** 상한 도달 카드는 `Lv.MAX`(또는 최대 레벨 숫자) 표시. 식물 스탯은 명시적 상한이 없으니 숫자만 계속 증가.
- **위치:** 식물 = `upStatCard`의 이름 옆/아래에 `.ust-lv` 배지. 탐사선 = 콜아웃 박스 헤더에 `Lv.N` 칩. 기존 현재값→강화후값 표시와 별개로 병기.
...
- **레벨 표시(§3.5):** 식물 강화창·탐사선 개조실 모두 미강화 카드가 **Lv.1**로 보이고, 1회 강화 후 **Lv.2**로 오르는지 실측(식물 `upStatCard`·탐사선 콜아웃 박스).

## 7. 검토 포인트 (구현/밸런스 시 결정)
- **강화 레벨 단위(§3.5)** — 현재 설계는 스탯/트랙별 독립 Lv. "식물/탐사선 전체 합산 1개 레벨"을 원하면 변경.
