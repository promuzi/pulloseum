...
4. **전투 버그 정리** — 포식 에너지 강탈·발광 에너지 회복의 상한을 `energyMax`로 통일하고 폐기된 독초 물약의 전투 경로를 제거.
5. **전투 UI 보강** — 손가락 인트로 제거, 블러 없는 중앙 카드 페이즈, 턴 표시, 카드/누적 기록 전환, 독립 판정 창과 드래그 가능한 전투 기록을 구현. 판정 연출 속도와 모바일 식물 크기도 다듬음.
